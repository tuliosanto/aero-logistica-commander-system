...
const Dashboard = ({
  ...
  const loadWaitlist = () => {
    ...
    const baseWaitlist = allWaitlist.filter((entry: CANWaitlistPassenger) => entry.baseAerea === currentUser.baseAerea);
  ...
  const handleSaveWaitlistEntry = (entry: Omit<CANWaitlistPassenger, 'id' | 'dataInscricao' | 'baseAerea'>) => {
    ...
    if (editingWaitlistPassenger) {
      ...
      const updatedWaitlist = allWaitlist.map((p: CANWaitlistPassenger) => p.id === editingWaitlistPassenger.id ? {
        ...editingWaitlistPassenger,
        ...entry
      } : p);
  ...
  const handleCompleteMission = (missionId: string) => {
    ...
    if (!mission) return;
    const updatedMissions = allMissions.map((m: Mission) => m.id === missionId ? {
      ...m,
      isCompleted: true
    } : m);
    localStorage.setItem('missions', JSON.stringify(updatedMissions));
    ...
    if (mission.passageiros && mission.passageiros.length > 0) {
      const allWaitlist = JSON.parse(localStorage.getItem('canWaitlist') || '[]');
      const waitlistIdsToRemove = mission.passageiros.filter(p => p.fromWaitlist && p.waitlistId).map(p => p.waitlistId);
      if (waitlistIdsToRemove.length > 0) {
        const updatedWaitlist = allWaitlist.filter((entry: CANWaitlistPassenger) => !waitlistIdsToRemove.includes(entry.id));
        localStorage.setItem('canWaitlist', JSON.stringify(updatedWaitlist));
        loadWaitlist();
      }
    }
      ...
      <nav className="aviation-nav px-6 py-4 sticky top-0 z-50 bg-[fab-blue-700] bg-blue-950">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <Button variant="outline" size="sm" onClick={onLogout} className="text-slate-600 hover:text-slate-800 hover:bg-red-50 hover:border-red-200 transition-all duration-200 font-semibold">
      ...
      <div className="container mx-auto p-6">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className={`grid w-full ${getTabsCount()} mb-6 aviation-tabs p-1 rounded-xl`}>
            {currentUser.perfil !== 'Secretario' && <TabsTrigger value="missions" className="flex items-center space-x-2 tab-missions transition-all duration-200 rounded-lg py-2 px-4 text-slate-50">
            ...
            <TabsTrigger value="waitlist" className="flex items-center space-x-2 tab-waitlist transition-all duration-200 rounded-lg py-2 px-4">
              <UserPlus className="w-4 h-4" />
              <span>Inscrições CAN</span>
            </TabsTrigger>
            {currentUser.perfil !== 'Operador' && currentUser.perfil !== 'Secretario' && <TabsTrigger value="users" className="flex items-center space-x-2 tab-users transition-all duration-200 rounded-lg py-2 px-4">
                <Users className="w-4 h-4" />
                <span>Usuários</span>
              </TabsTrigger>}
            {currentUser.perfil !== 'Operador' && currentUser.perfil !== 'Secretario' && <TabsTrigger value="config" className="flex items-center space-x-2 tab-config transition-all duration-200 rounded-lg py-2 px-4">
          ...
          {currentUser.perfil !== 'Secretario' && <TabsContent value="missions" className="space-y-6">
              <div className="flex justify-between items-center">
                ...
                <Button onClick={handleCreateMission} className="aviation-button text-white px-6 py-3 text-base rounded-xl">
                  <PlusCircle className="w-5 h-5 mr-2" />
                  Nova Missão
                </Button>
              </div>

              {showMissionForm ? <Card className="aviation-card rounded-xl">
                ...
                setShowMissionForm(false);
                setEditingMission(null);
              }} currentUser={currentUser} waitlist={waitlist} onUpdateWaitlist={loadWaitlist} />
                  </CardContent>
                </Card> : <div className="aviation-card rounded-xl p-6">
                  <MissionList missions={missions} onEdit={handleEditMission} onDelete={handleDeleteMission} onComplete={handleCompleteMission} onArchive={handleArchiveMission} currentUser={currentUser} />
                </div>}
            </TabsContent>}

          {currentUser.perfil !== 'Secretario' && <TabsContent value="archived" className="space-y-6">
              <div>
              ...
              <div className="aviation-card rounded-xl p-6">
                <ArchivedMissions missions={archivedMissions} currentUser={currentUser} />
              </div>
            </TabsContent>}
          ...
          <TabsContent value="waitlist" className="space-y-6">
            ...
            {showWaitlistForm ? <Card className="aviation-card rounded-xl">
                <CardHeader>
                  <CardTitle>
                ...
                <CardContent>
                  <CANWaitlistForm passenger={editingWaitlistPassenger} onSave={handleSaveWaitlistEntry} onCancel={() => {
                setShowWaitlistForm(false);
                setEditingWaitlistPassenger(null);
              }} currentUser={currentUser} />
                </CardContent>
              </Card> : <div className="aviation-card rounded-xl p-6">
                <CANWaitlist waitlist={waitlist} onAddToMission={handleAddToMission} onRemove={handleRemoveFromWaitlist} onEdit={handleEditWaitlistPassenger} />
              </div>}
          </TabsContent>
